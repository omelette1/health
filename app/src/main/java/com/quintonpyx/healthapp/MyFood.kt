...
//                    Log.d("USERS", userSnapshot.value)
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
               ...
               if(snapshot.exists()){
                   for(usr in snapshot.children){
                       val user = usr.getValue(User::class.java) as User
                       txtUnit.text = "/ "+user.targetCalorie +" kcal"
                   }
        ...
        database.child("user").orderByChild("uid").equalTo(user.uid).addListenerForSingleValueEvent(userListener)
